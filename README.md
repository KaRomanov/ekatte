...
![Схема](./src/schema.png)

Има 4 таблици - за селища, кметства, общини и региони. 

Не всяко селище е част от кметство, затова `townhall_id` в `towns` таблицата може да бъде __NULL__. Структурата позволява лесен достъп чрез foreign keys. От towns можем да съединим таблцата с townhalls и така да видим информация за кметството на селището. Можем така да продължим нагоре по йерархията и да съединим с общините чрез __municipality_id__ и __id__ в таблицата на общините. Аналогично и за регионите. Така за едно селище можем да извлечем цялата информация.

Всички __primary key__ са индексирани, за да бъдат по-бързи join-овете на таблиците и има foreign key за връзките между таблиците.
